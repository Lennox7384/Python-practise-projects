# Bubble sort is a sorting algorithm that repeatedly compares two adjascent items and swaps them to be in desired order. 
...
def bubbleSortAlg (array): # Create a function with a descriptive name for the type of sort algorithm, in this case bubble sort.
    ...
    for i in range (n): #Range(n) generates a sequence of numbers from 0 to n -1.
        ...
        for j in range (0, n-i-1): # Reduces maximum possible iterations of j by 1 successively for each ith iteration. 
...
bubbleSortAlg(arr) #Run the function. 
